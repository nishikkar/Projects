...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        revision=False
        ylist=self.domains[y].copy()
        xlist=self.domains[x].copy()
        if self.crossword.overlaps[x,y] != None:
            i=int(self.crossword.overlaps[x,y][0])
            j=int(self.crossword.overlaps[x,y][1])
            for word in xlist:
                found=False
                for wordy in ylist:
                        if word[i]==wordy[j]:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for v in self.crossword.variables:
            if v not in assignment:
                if len(self.domains[v])<min:
                    minvar=v
                    min=len(self.domains[v])
        return minvar
        raise NotImplementedError
